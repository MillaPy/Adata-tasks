...
class GosZakup():
    ...
    def chrome_exe_dynamic_path_search(self):
        ...
        # thus need to find in your device path to chrome extension
        main_path = "C:/"
        # it will waklk through above folder to find extension in folders
        for root, dirs, files in os.walk(main_path):
            if chrome_exe in files:
                chrome_path = os.path.join(root, chrome_exe)
                # if path contains words such as google it will show it
                if search("Google", str(chrome_path)) or search("google", str(chrome_path)):
                    print("\n-> Found your chrome-path: ", chrome_path)
